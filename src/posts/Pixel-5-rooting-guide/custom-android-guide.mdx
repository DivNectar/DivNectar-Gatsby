...
19. Press enter on your keyboard and you should recieve a confirmation that the file was flashed correctly
...
3. flash custom kernel (if need be, this is entirely optional)
...
4. reboot your phone into bootloader mode using ```sh adb reboot bootloader```
5. Once the phone has re-booted, run ```sh fastboot -w``` to **wipe** the phone's data
6. Run ```sh ./flash-all.sh``` **do not touch the phone during this process**
